...
class ContentInterviewSkill(InterviewSkill):
    ...
    def interview(self, unit: ContentUnit) -> Material:
        """执行内容访谈，产出结构化素材库."""
    ...
    def run(self, unit: ContentUnit, **kwargs: object) -> ContentUnit:
        """执行内容访谈 Skill."""
        material = self.interview(unit)
        unit.move("interviewing", f"完成内容访谈，收集 {sum(len(getattr(material, f)) for f in material.model_fields)} 条素材")
        return unit
